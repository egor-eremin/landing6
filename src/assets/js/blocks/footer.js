...
(function() {
    ...
    let fDatas = {
        ...
        copy: {
            ru: "<span>Защищено SSL.</span> <span>Авторское право © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". Все права защищены.</span>",
            en: "<span>Secured by SSL.</span> <span>Copyright © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". All rights reserved.</span>",
            es: "<span>Asegurado por SSL.</span> <span>Copyright © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". Todos los derechos reservados.</span>",
            de: "<span>Gesichert durch SSL.</span> <span>Urheberrecht © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". Alle Rechte vorbehalten.</span>",
            fr: "<span>Sécurisé par SSL.</span> <span>Copyright © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". Tous droits réservés.</span>",
            it: "<span>Protetto da SSL.</span> <span>Copyright © " + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + ". Tutti i diritti riservati.</span>",
            zh: "<span>受SSL保護。</span><span>版權所有©" + (vars.getCurrentYear() - 3) + " - " + vars.getCurrentYear() + " " + vars.name + "。版權所有</span>",
        },
    };
